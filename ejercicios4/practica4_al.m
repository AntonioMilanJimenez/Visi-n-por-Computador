...
hold on;
plot(C(:,1),C(:,2),'r*');
%Se consigue detectar todos los cruces del tablero
...
h = imfilter(I, mascaraH, 'replicate');
v = imfilter(I, mascaraV, 'replicate');
diag1 = imfilter(I, mascaraDiag1, 'replicate');
diag2 = imfilter(I, mascaraDiag2, 'replicate');

%Se aplica la mascara de media 3x3
mascara = fspecial('average',3);
hh = imfilter(h, mascara, 'replicate');
vv = imfilter(v, mascara, 'replicate');
d1 = imfilter(diag1, mascara, 'replicate');
d2 = imfilter(diag2, mascara, 'replicate');

%Se crea una nueva imagen con el minimo de hh, vv, d1 y d2
aux = min(hh,vv);
aux = min(aux,d1);
c = min(aux,d2);
...
%Para hacerlos más notarios podemos multiplicarlos por una constante
%sabiendo que el resto de los pixeles están a 0
figure,imshow(c.*5),title('Esquinas (Moravec)');

%Y visualizarlos sobre la imagen
figure,imshow(c.*5 + I),title('Esquinas (Moravec)');
